...
namespace zonetool::h2
{
	...
	zonetool::h1::GfxStateBitsRaw parse_statebits_raw_data(const ordered_json& data, const std::string& name)
	{
...
#define READ_STR_LB_FROM_JSON(x) std::string x = data[#x].get<std::string>();
		...
		loadbits3 |= srcBlendRgb << zonetool::h1::GFXS3_SRCBLEND_RGB_SHIFT;
		loadbits3 |= dstBlendRgb << zonetool::h1::GFXS3_DSTBLEND_RGB_SHIFT;
		loadbits3 |= blendOpRgb << zonetool::h1::GFXS3_BLENDOP_RGB_SHIFT;
		loadbits3 |= srcBlendAlpha << zonetool::h1::GFXS3_SRCBLEND_ALPHA_SHIFT;
		loadbits3 |= dstBlendAlpha << zonetool::h1::GFXS3_DSTBLEND_ALPHA_SHIFT;
		loadbits3 |= blendOpAlpha << zonetool::h1::GFXS3_BLENDOP_ALPHA_SHIFT;

		if (depthTest == "always")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_ALWAYS;
		}
		else if (depthTest == "less")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_LESS;
		}
		else if (depthTest == "equal")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_EQUAL;
		}
		else if (depthTest == "lessequal")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_LESSEQUAL;
		}
		else if (depthTest == "great")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_GREAT;
		}
		else if (depthTest == "greatequal")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_GREATEQUAL;
		}
		else if (depthTest == "disable")
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHTEST_DISABLE;
		}
		else
		{
			__debugbreak();
		...
		{
			loadbits0 |= zonetool::h1::GFXS0_ATEST_DISABLE;
		}
		else if (alphaTest == ">0")
		{
			loadbits0 |= zonetool::h1::GFXS0_ATEST_GT_0;
		}
		else if (alphaTest == "<128")
		{
			loadbits0 |= zonetool::h1::GFXS0_ATEST_LT_128;
		}
		else if (alphaTest == ">=128")
		{
			loadbits0 |= zonetool::h1::GFXS0_ATEST_GE_128;
		}
		else
		{
		...
		if (cullFace == "none")
		{
			loadbits0 |= zonetool::h1::GFXS0_CULL_NONE;
		}
		else if (cullFace == "back")
		{
			loadbits0 |= zonetool::h1::GFXS0_CULL_BACK;
		}
		else if (cullFace == "front")
		{
			loadbits0 |= zonetool::h1::GFXS0_CULL_FRONT;
		}
		else
		{
		...
		if (gammaWrite)
		{
			loadbits0 |= zonetool::h1::GFXS0_GAMMAWRITE;
		}

		if (colorWriteAlpha)
		{
			loadbits3 |= zonetool::h1::GFXS3_BLEND_COLORWRITE_A;
		}
		if (colorWriteR)
		{
			loadbits3 |= zonetool::h1::GFXS3_BLEND_COLORWRITE_R;
		}
		if (colorWriteG)
		{
			loadbits3 |= zonetool::h1::GFXS3_BLEND_COLORWRITE_G;
		}
		if (colorWriteB)
		{
			loadbits3 |= zonetool::h1::GFXS3_BLEND_COLORWRITE_B;
		}

		if (polymodeLine)
		{
			loadbits0 |= zonetool::h1::GFXS0_POLYMODE_LINE;
		}

		if (depthWrite)
		{
			loadbits1 |= zonetool::h1::GFXS1_DEPTHWRITE;
		}
		if (depthWriteOpaque)
		{
			loadbits3 |= zonetool::h1::GFXS3_DEPTHWRITE_OPAQUE;
		}

		if (stencilFrontEnabled)
		{
			loadbits1 |= zonetool::h1::GFXS1_STENCIL_FRONT_ENABLE;
		}
		if (stencilBackEnabled)
		{
			loadbits1 |= zonetool::h1::GFXS1_STENCIL_BACK_ENABLE;
		}

		loadbits1 |= stencilFrontPass << zonetool::h1::GFXS1_STENCIL_FRONT_PASS_SHIFT;
		loadbits1 |= stencilFrontFail << zonetool::h1::GFXS1_STENCIL_FRONT_FAIL_SHIFT;
		loadbits1 |= stencilFrontZFail << zonetool::h1::GFXS1_STENCIL_FRONT_ZFAIL_SHIFT;
		loadbits1 |= stencilFrontFunc << zonetool::h1::GFXS1_STENCIL_FRONT_FUNC_SHIFT;
		loadbits1 |= stencilBackPass << zonetool::h1::GFXS1_STENCIL_BACK_PASS_SHIFT;
		loadbits1 |= stencilBackFail << zonetool::h1::GFXS1_STENCIL_BACK_FAIL_SHIFT;
		loadbits1 |= stencilBackZFail << zonetool::h1::GFXS1_STENCIL_BACK_ZFAIL_SHIFT;
		loadbits1 |= stencilBackFunc << zonetool::h1::GFXS1_STENCIL_BACK_FUNC_SHIFT;

		//loadbits2 |= stencilRefBits;

		zonetool::h1::GfxStateBitsRaw bits{};
	...
	{
		zonetool::h1::GfxStateBitsRaw entry{};
		entry.flags.loadbit0 = static_cast<zonetool::h1::GfxStateBitRawBits0>(statebits->loadBits[zonetool::h1::GFXS_RAWBITS_CULL_INDEX]);
		entry.flags.loadbit1 = static_cast<zonetool::h1::GfxStateBitRawBits1>(statebits->loadBits[zonetool::h1::GFXS_RAWBITS_DEPTH_STENCIL_INDEX]);
		entry.flags.loadbit2 = static_cast<zonetool::h1::GfxStateBitRawBits2>(statebits->loadBits[zonetool::h1::GFXS_RAWBITS_UNK2_INDEX]);
		entry.flags.loadbit3 = static_cast<zonetool::h1::GfxStateBitRawBits3>(statebits->loadBits[zonetool::h1::GFXS_RAWBITS_BLEND_INDEX]);
		entry.flags.loadbit4 = static_cast<zonetool::h1::GfxStateBitRawBits4>(statebits->loadBits[zonetool::h1::GFXS_RAWBITS_UNK4_INDEX]);
		entry.flags.loadbit5 = static_cast<zonetool::h1::GfxStateBitRawBits5>(statebits->loadBits[zonetool::h1::GFXS_RAWBITS_UNK5_INDEX]);

		const auto srcBlendRgb = (entry.flags.loadbit3 & zonetool::h1::GFXS3_SRCBLEND_RGB_MASK) >> zonetool::h1::GFXS3_SRCBLEND_RGB_SHIFT;
		const auto dstBlendRgb = (entry.flags.loadbit3 & zonetool::h1::GFXS3_DSTBLEND_RGB_MASK) >> zonetool::h1::GFXS3_DSTBLEND_RGB_SHIFT;
		const auto blendOpRgb = (entry.flags.loadbit3 & zonetool::h1::GFXS3_BLENDOP_RGB_MASK) >> zonetool::h1::GFXS3_BLENDOP_RGB_SHIFT;
		const auto srcBlendAlpha = (entry.flags.loadbit3 & zonetool::h1::GFXS3_SRCBLEND_ALPHA_MASK) >> zonetool::h1::GFXS3_SRCBLEND_ALPHA_SHIFT;
		const auto dstBlendAlpha = (entry.flags.loadbit3 & zonetool::h1::GFXS3_DSTBLEND_ALPHA_MASK) >> zonetool::h1::GFXS3_DSTBLEND_ALPHA_SHIFT;
		const auto blendOpAlpha = (entry.flags.loadbit3 & zonetool::h1::GFXS3_BLENDOP_ALPHA_MASK) >> zonetool::h1::GFXS3_BLENDOP_ALPHA_SHIFT;

		const auto polygonOffset = (entry.flags.loadbit1 & zonetool::h1::GFXS1_POLYGON_OFFSET_MASK);

		const auto* depthTest = "always";
		if ((entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHTEST_MASK) == zonetool::h1::GFXS1_DEPTHTEST_LESS)
		{
			depthTest = "less";
		}
		else if ((entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHTEST_MASK) == zonetool::h1::GFXS1_DEPTHTEST_EQUAL)
		{
			depthTest = "equal";
		}
		else if ((entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHTEST_MASK) == zonetool::h1::GFXS1_DEPTHTEST_LESSEQUAL)
		{
			depthTest = "lessequal";
		}
		else if ((entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHTEST_MASK) == zonetool::h1::GFXS1_DEPTHTEST_GREAT)
		{
			depthTest = "great";
		}
		else if ((entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHTEST_MASK) == zonetool::h1::GFXS1_DEPTHTEST_GREATEQUAL)
		{
			depthTest = "greatequal";
		}
		else if ((entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHTEST_MASK) == zonetool::h1::GFXS1_DEPTHTEST_DISABLE)
		{
			depthTest = "disable";
		}

		const auto* alphaTest = "disable";
		if ((entry.flags.loadbit0 & zonetool::h1::GFXS0_ATEST_MASK) == zonetool::h1::GFXS0_ATEST_GE_128)
		{
			alphaTest = ">=128";
		}
		else if ((entry.flags.loadbit0 & zonetool::h1::GFXS0_ATEST_MASK) == zonetool::h1::GFXS0_ATEST_GT_0)
		{
			alphaTest = ">0";
		}
		else if ((entry.flags.loadbit0 & zonetool::h1::GFXS0_ATEST_MASK) == zonetool::h1::GFXS0_ATEST_LT_128)
		{
			alphaTest = "<128";
		}
		else
		...
		if ((entry.flags.loadbit0 & zonetool::h1::GFXS0_CULL_MASK) == zonetool::h1::GFXS0_CULL_BACK)
		{
			cullFace = "back";
		}
		else if ((entry.flags.loadbit0 & zonetool::h1::GFXS0_CULL_MASK) == zonetool::h1::GFXS0_CULL_FRONT)
		{
			cullFace = "front";
		}
		else
		{
			assert((entry.flags.loadbit0 & zonetool::h1::GFXS0_CULL_MASK) == zonetool::h1::GFXS0_CULL_NONE);
		}

		const auto colorWriteR = entry.flags.loadbit3 & zonetool::h1::GFXS3_BLEND_COLORWRITE_R ? true : false;
		const auto colorWriteG = entry.flags.loadbit3 & zonetool::h1::GFXS3_BLEND_COLORWRITE_G ? true : false;
		const auto colorWriteB = entry.flags.loadbit3 & zonetool::h1::GFXS3_BLEND_COLORWRITE_B ? true : false;
		const auto colorWriteAlpha = entry.flags.loadbit3 & zonetool::h1::GFXS3_BLEND_COLORWRITE_A ? true : false;
		const auto polymodeLine = entry.flags.loadbit0 & zonetool::h1::GFXS0_POLYMODE_LINE ? true : false;
		const auto gammaWrite = entry.flags.loadbit0 & zonetool::h1::GFXS0_GAMMAWRITE ? true : false;
		const auto depthWrite = (entry.flags.loadbit1 & zonetool::h1::GFXS1_DEPTHWRITE) ? true : false;
		const auto depthWriteOpaque = (entry.flags.loadbit3 & zonetool::h1::GFXS3_DEPTHWRITE_OPAQUE) ? true : false;
		const auto stencilFrontEnabled = (entry.flags.loadbit1 & zonetool::h1::GFXS1_STENCIL_FRONT_ENABLE) ? true : false;
		const auto stencilBackEnabled = (entry.flags.loadbit1 & zonetool::h1::GFXS1_STENCIL_BACK_ENABLE) ? true : false;
		const auto stencilFrontPass = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_FRONT_PASS_SHIFT) & zonetool::h1::GFXS_STENCILOP_MASK;
		const auto stencilFrontFail = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_FRONT_FAIL_SHIFT) & zonetool::h1::GFXS_STENCILOP_MASK;
		const auto stencilFrontZFail = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_FRONT_ZFAIL_SHIFT) & zonetool::h1::GFXS_STENCILOP_MASK;
		const auto stencilFrontFunc = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_FRONT_FUNC_SHIFT) & zonetool::h1::GFXS_STENCILFUNC_MASK;
		const auto stencilBackPass = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_BACK_PASS_SHIFT) & zonetool::h1::GFXS_STENCILOP_MASK;
		const auto stencilBackFail = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_BACK_FAIL_SHIFT) & zonetool::h1::GFXS_STENCILOP_MASK;
		const auto stencilBackZFail = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_BACK_ZFAIL_SHIFT) & zonetool::h1::GFXS_STENCILOP_MASK;
		const auto stencilBackFunc = (entry.flags.loadbit1 >> zonetool::h1::GFXS1_STENCIL_BACK_FUNC_SHIFT) & zonetool::h1::GFXS_STENCILFUNC_MASK;

		const auto stencilRefBits = (entry.flags.loadbit2 & zonetool::h1::GFXS2_STENCIL_REF_MASK);
