...
namespace zonetool::h2
{
	...
	void scriptable_def::prepare_scriptable_event_def(ScriptableEventDef* event, zone_buffer* buf, zone_memory* mem)
	{
		...
		{
		case SCRIPTABLE_EVENT_FX:
			event->data.playFx.tagName = static_cast<scr_string_t>(buf->write_scriptstring(
				this->get_script_string(&event->data.playFx.tagName)));
			break;
		case SCRIPTABLE_EVENT_STOP_FX:
			event->data.stopFx.tagName = static_cast<scr_string_t>(buf->write_scriptstring(
				this->get_script_string(&event->data.stopFx.tagName)));
			break;
		default:
			break;
		}
	...
	void scriptable_def::prepare(zone_buffer* buf, zone_memory* mem)
	{
		...
		for (unsigned char i = 0; i < data->partCount; i++)
		{
			...
			data->parts[i].name = static_cast<scr_string_t>(buf->write_scriptstring(
				this->get_script_string(&data->parts[i].name)));
		}
