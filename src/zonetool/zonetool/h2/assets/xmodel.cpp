...
namespace zonetool::h2
{
	...
	void xmodel::prepare(zone_buffer* buf, zone_memory* mem)
	{
		...
		if (xmodel->weightNames)
		{
			...
			{
				xmodel->weightNames[i] = static_cast<scr_string_t>(buf->write_scriptstring(
					this->get_script_string(&xmodel->weightNames[i])));
			}
		}
	}
	...
	void xmodel::build_composite_model(const std::string& name, std::vector<std::string> attachments)
	{
