...
namespace zonetool::h1
{
	namespace converter::s1
	{
		namespace sound
		{
			...
			{
				const auto new_asset = allocator.allocate<zonetool::s1::snd_alias_list_t>();

				std::memcpy(new_asset, asset, sizeof(snd_alias_list_t));
				new_asset->head = allocator.allocate_array<zonetool::s1::snd_alias_t>(asset->count);
				...
				for (auto i = 0u; i < asset->count; i++)
				{
					...
					const auto convert_and_dump_streamed = [&]() -> bool
					{
						...
						new_head->soundFile->type = zonetool::s1::SAT_LOADED;

						SoundAliasFlags flags{0};
						flags.intValue = new_head->flags;
						flags.packed.type = zonetool::s1::SAT_LOADED;
						new_head->flags = flags.intValue;

						new_head->soundFile->u.loadSnd = allocator.allocate<zonetool::s1::LoadedSound>();
					...
					if (head->soundFile)
					{
						if (head->soundFile->type == SAT_STREAMED)
						{
							if (!convert_and_dump_streamed())
							{
								...
								new_head->soundFile->type = zonetool::s1::SAT_STREAMED;
			...
			void dump(snd_alias_list_t* asset)
			{
				...
				zonetool::s1::sound::dump(converted_asset);
			}
		}
	}
}
