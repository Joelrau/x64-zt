...
#include "zonetool/s1/assets/material.hpp"
	...
	namespace converter::s1
	{
		namespace material
		{
			zonetool::s1::Material* convert(Material* asset, utils::memory::allocator& allocator)
			{
				...
				std::memset(new_asset->stateBitsEntry, 0xFF, zonetool::s1::MaterialTechniqueType::TECHNIQUE_COUNT);
				std::memcpy(new_asset->stateBitsEntry, asset->stateBitsEntry, zonetool::s1::MaterialTechniqueType::TECHNIQUE_COUNT);
				...
				new_asset->techniqueSet = allocator.allocate<zonetool::s1::MaterialTechniqueSet>();
				...
				std::memset(new_asset->constantBufferIndex, 0xFF, zonetool::s1::MaterialTechniqueType::TECHNIQUE_COUNT);
				std::memcpy(new_asset->constantBufferIndex, asset->constantBufferIndex, zonetool::s1::MaterialTechniqueType::TECHNIQUE_COUNT);
			...
			void dump(Material* asset)
			{
				...
				zonetool::s1::material::dump(converted_asset);
			}
		}
	}
}
