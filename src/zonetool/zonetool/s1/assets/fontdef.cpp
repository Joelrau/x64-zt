...
namespace zonetool::s1
{
		...
		const std::string& techniqueset, const uint8_t sort_key, const uint8_t camera_region, zone_memory* mem)
	{
		...
		mat->textureTable[0].samplerState = 243;
...
#define READ_CONFIG_VAL(__name__, __type__) \
	...
	Font_s* font_def::parse_ttf(const std::string& name, zone_memory* mem)
	{
		...
		const auto techniqueset = "2d_pd";
		const auto sortkey = 52;
		const auto sortkey_glow = 38;
		...
		font->glowMaterial = generate_material(name + "_glow", image, techniqueset, sortkey_glow, camera_region, mem);
