...
namespace zonetool::iw6
{
	...
	Material* IMaterial::parse(std::string name, ZoneMemory* mem)
	{
		...
		if (mat->techniqueSet)
		{
			...
			ITechset::parse_statebitsmap(mat->techniqueSet->name, &mat->stateMap, &mat->stateBitsCount,
	...
	void IMaterial::dump(Material* asset)
	{
		...
		if (asset)
		{
			...
			if (asset && asset->techniqueSet)
			{
				...
				ITechset::dump_statebits_map(asset->techniqueSet->name, asset->stateMap, asset->stateBitsCount);
