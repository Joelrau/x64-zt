...
namespace zonetool::iw8
{
	...
	{
		const auto asset_path = utils::string::va("streamed_images\\%s.iw8Image", clean_name(name).data());
	...
	{
		auto path = "images\\" + clean_name(name) + ".iw8Image";
	...
	void dump_streamed_image(GfxImage* image, bool is_self = false, bool dump_dds = false)
	{
		...
		{
			const auto path = "streamed_images\\"s + clean_name(image->name) + ".iw8Image"s;
	...
	void gfx_image::dump(GfxImage* asset)
	{
		...
		auto path = "images\\"s + clean_name(asset->name) + ".iw8Image"s;
		assetmanager::dumper write;
		if (!write.open(path))
		{
			return;
