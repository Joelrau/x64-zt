...
namespace zonetool::iw8
{
	...
	void scriptable_def::parse_state(assetmanager::reader& read, ScriptableStateDef* data)
	{
		...
		case Scriptable_StateType_Usable:
			data->data.usable.base = read.read_single<ScriptableStateBaseDef>();
			if (data->data.usable.base)
			{
				parse_state_base(read, data->data.usable.base);
			}
			data->data.usable.script_id = read.read_string();
			this->add_script_string(&data->data.usable.scrScript_id, read.read_string());
			break;
	...
	void scriptable_def::prepare_state(zone_buffer* buf, ScriptableStateDef* data)
	{
		...
		case Scriptable_StateType_Usable:
			if (data->data.usable.base)
			{
				prepare_state_base(buf, data->data.usable.base);
			}
			data->data.usable.scrScript_id = static_cast<scr_string_t>(buf->write_scriptstring(
				this->get_script_string(&data->data.usable.scrScript_id)));
	...
	void scriptable_def::load_depending_state(zone_base* zone, ScriptableStateDef* data)
	{
		...
		else if (data->type == Scriptable_StateType_Usable)
		{
			if (data->data.usable.base)
			{
	...
	void write_state(zone_base* zone, zone_buffer* buf, ScriptableStateDef* data, ScriptableStateDef* dest)
	{
		...
		else if (data->type == Scriptable_StateType_Usable)
		{
			...
			{
				buf->align(7);
				dest->data.usable.base = buf->write(data->data.usable.base);
				write_state_base(zone, buf, data->data.usable.base, dest->data.usable.base);
				buf->clear_pointer(&dest->data.usable.base);
			}

			if (data->data.usable.script_id)
			{
				dest->data.usable.script_id = buf->write_str(data->data.usable.script_id);
			}
		}
	}
	...
	void scriptable_def::dump_state(assetmanager::dumper& dump, ScriptableStateDef* data)
	{
		...
		{
			dump.dump_single(data->data.usable.base);
			if (data->data.usable.base)
			{
				dump_state_base(dump, data->data.usable.base);
			}
			dump.dump_string(data->data.usable.script_id);
			dump.dump_string(SL_ConvertToString(data->data.usable.scrScript_id));
		}
	}
