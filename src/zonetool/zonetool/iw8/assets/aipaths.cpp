	...
	*/
	...
	PathData* path_data::parse(const std::string& name, zone_memory* mem)
	{
		...
		for (auto i = 0u; i < asset->nodeCount; i++)
		{
			...
			ADD_SCRIPT_STRING(interactable);
	...
	void path_data::prepare(zone_buffer* buf, zone_memory* mem)
	{
		...
		for (auto i = 0u; i < data->nodeCount; i++)
		{
			...
			WRITE_SCRIPT_STRING(interactable);
	...
	void path_data::dump(PathData* asset)
	{
		...
		for (auto i = 0u; i < asset->nodeCount; i++)
		{
			auto test = asset->nodes[i];
			auto test2 = asset->nodes[i].constant;
			...
			dumper.dump_string(SL_ConvertToString(asset->nodes[i].constant.interactable));

			if (!asset->parentIndexResolved) // DB_PathDataParentIndexResolved
			{
