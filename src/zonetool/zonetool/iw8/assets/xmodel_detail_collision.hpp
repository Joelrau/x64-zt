...
{
	class xmodel_detail_collision : public asset_interface
	{
	private:
		std::string name_;
		XModelDetailCollision* asset_ = nullptr;
	...
	public:
		XModelDetailCollision* parse(const std::string& name, zone_memory* mem);
		...
		static void dump(XModelDetailCollision* asset);
	};
}