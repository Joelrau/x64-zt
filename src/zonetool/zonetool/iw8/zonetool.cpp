...
#undef DUMP_ASSET_REGULAR
	...
	void dump_asset_iw8(XAsset* asset)
	{
...
#undef DUMP_ASSET
			...
			else if (row->fields[0] == "iterate"s)
			{
				if (row->num_fields >= 2)
				{
					...
					try
					{
						...
						{
							add_assets_using_iterator(fastfile, type, "particlesystem", ".iw8VFX", true, zone);
						}
	...
	dump_params get_dump_params(const ::iw8::command::params& params)
	{
		dump_params dump_params{};
		dump_params.target = game::iw8;
	...
	{
		::iw8::command::add("quit", []()
		{
			std::quick_exit(EXIT_SUCCESS);
		});

		::iw8::command::add("buildzone", [](const ::iw8::command::params& params)
		{
			if (params.size() != 2)
			{
		...
		::iw8::command::add("loadzone", [](const ::iw8::command::params& params)
		{
			if (params.size() != 2)
			{
		...
		::iw8::command::add("unloadzones", []()
		{
			unload_zones();
		});

		::iw8::command::add("dumpzone", [](const ::iw8::command::params& params)
		{
			if (params.size() < 2)
			{
			...
			{
				dump_zone(params.get(1), game::iw8);
			}
		});

		::iw8::command::add("dumpasset", [](const ::iw8::command::params& params)
		{
			...
			asset.header = header;
			globals.target_game = game::iw8;
			dump_asset(&asset);

			ZONETOOL_INFO("Dumped to dump/assets");
		});

		::iw8::command::add("dumpcsv", [](const ::iw8::command::params& params)
		{
			if (params.size() != 2)
			{
		...
		::iw8::command::add("verifyzone", [](const ::iw8::command::params& params)
		{
			if (params.size() != 2)
			{
		...
		::iw8::command::add("generatecsv", csv_generator::create_command
			<::iw8::command::params>([](const uint32_t id)
		{
			return gsc::iw8::gsc_ctx->token_name(id);
		}));

		::iw8::command::add("iteratezones", []()
		{
			iterate_zones();
		});
	}
