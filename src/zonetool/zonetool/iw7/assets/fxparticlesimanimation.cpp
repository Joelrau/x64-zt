...
namespace zonetool::iw7
{
	FxParticleSimAnimation* fx_particle_sim_animation::parse(const std::string& name, zone_memory* mem)
	{
		...
		if (asset->particleDataOrientation3D)
		{
			asset->particleDataOrientation3D = read.read_array<FxParticleSimAnimationParticleDataOrientation3D>();
		}
		if (asset->particleDataIndex)
		{
			asset->particleDataIndex = read.read_array<FxParticleSimAnimationParticleDataIndex>();
		}
		if (asset->frames)
		{
		...
		if (asset->colorTable)
		{
			asset->colorTable = read.read_array<vec4_t>();
		}
	...
	void fx_particle_sim_animation::write(zone_base* zone, zone_buffer* buf)
	{
		...
		if (data->particleDataOrientation3D)
		{
			buf->align(7);
			buf->write(data->particleDataOrientation3D, data->header.particleDataCount);
			buf->clear_pointer(&dest->particleDataOrientation3D);
		}
		if (data->particleDataIndex)
		{
			buf->align(1);
			if (data->header.hasIndex)
			{
				buf->write(data->particleDataIndex, data->header.particleDataCount);
				buf->clear_pointer(&dest->particleDataIndex);
			}
			else
			{
				dest->particleDataIndex = nullptr;
			}
		}
		if (data->frames)
		{
	...
	void fx_particle_sim_animation::dump(FxParticleSimAnimation* asset)
	{
		...
		if (asset->particleDataOrientation3D)
		{
			dump.dump_array(asset->particleDataOrientation3D, asset->header.particleDataCount);
		}
		if (asset->particleDataIndex)
		{
			dump.dump_array(asset->particleDataIndex, asset->header.particleDataCount);
		}
		if (asset->frames)
		{
