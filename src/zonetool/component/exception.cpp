...
namespace exception
{
	namespace
	{
		...
		void display_error_dialog()
		{
			...
			MessageBoxA(nullptr, error_str.data(), "ZoneTool ERROR", MB_ICONERROR);
		...
		std::string generate_crash_info(const LPEXCEPTION_POINTERS exceptioninfo)
		{
			...
			line("ZoneTool Crash Dump");
			line("");
			line("Game: "s + game::get_mode_as_string());
...
#pragma warning(pop)
		...
		{
			const std::string crash_name = utils::string::va("minidumps/zonetool-crash-%s.zip",
				get_timestamp().data());

			utils::compression::zip::archive zip_file{};
			zip_file.add("crash.dmp", ::exception::create_minidump(exceptioninfo));
			zip_file.add("info.txt", generate_crash_info(exceptioninfo));
			zip_file.write(crash_name, "ZoneTool Crash Dump");
		}
