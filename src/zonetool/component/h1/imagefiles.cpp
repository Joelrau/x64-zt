	...
	namespace imagefiles
	{
		namespace
		{
			...
			namespace mp
			{
				...
				static_assert(sizeof(image_file_unk) == 120);
				...
				void* get_image_file_unk_mp(unsigned int index)
				{
					...
					{
						return &reinterpret_cast<image_file_unk*>(0x145332C90)[index];
					}
			...
			game::DB_IFileSysFile* get_image_file_handle(unsigned int index)
			{
				if (index != CUSTOM_IMAGE_FILE_INDEX)
				{
					return reinterpret_cast<game::DB_IFileSysFile**>(0x145123B20)[index];
				}
		...
		public:
			...
			{
				utils::hook::jump(0x1402C5A05, utils::hook::assemble(db_create_gfx_image_stream_stub), true);
				utils::hook::call(0x1402C5A47, pakfile_open_stub);
				utils::hook::call(0x1402C5A29, com_sprintf_stub);
			}
		};
	}
}
