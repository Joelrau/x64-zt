...
namespace h2
{
	namespace imagefiles
	{
		namespace
		{
			...
			std::unordered_map<std::string, h2::game::DB_IFileSysFile*> image_file_handles;
			...
			h2::game::DB_IFileSysFile* get_image_file_handle(unsigned int index)
			{
				if (index != CUSTOM_IMAGE_FILE_INDEX)
				{
					return reinterpret_cast<h2::game::DB_IFileSysFile**>(0x14417B500)[index];
				}
				...
				unsigned int index, int is_localized)
			{
				...
				const auto handle = db_fs->vftbl->OpenFile(db_fs, h2::game::SF_PAKFILE, utils::string::va("%s.pak", name.data()));
				if (handle != nullptr)
				{
...
REGISTER_COMPONENT_H2(h2::imagefiles::component)
