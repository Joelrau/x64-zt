...
namespace exception
{
	namespace
	{
		...
		void display_error_dialog()
		{
			...
			MessageBoxA(nullptr, error_str.data(), "S1-ZoneTool ERROR", MB_ICONERROR);
...
#pragma warning(pop)
		...
		{
			const std::string crash_name = utils::string::va("minidumps/s1-zonetool-crash-%s.zip",
			                                                 get_timestamp().data());

			utils::compression::zip::archive zip_file{};
			zip_file.add("crash.dmp", create_minidump(exceptioninfo));
			zip_file.add("info.txt", generate_crash_info(exceptioninfo));
			zip_file.write(crash_name, "S1-ZoneTool Crash Dump");
		}
